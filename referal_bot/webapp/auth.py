...
def validate_init_data(init_data: str, max_age_seconds: int = 86400):
    ...
    if not received_hash:
        print("DEBUG: hash yo'q initData ichida")
        return None
    ...
    if not hmac.compare_digest(calculated_hash, received_hash):
        print(f"DEBUG: hash mos kelmadi. BOT_TOKEN boshi: {BOT_TOKEN[:10]}... hisoblangan={calculated_hash[:10]} kelgan={received_hash[:10]}")
        return None
